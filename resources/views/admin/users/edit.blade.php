    ...
    <div class="card-body">
        <form action="{{ route("admin.users.update", [$user->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" required id="name" name="name" class="form-control" value="{{ old('name', isset($user) ? $user->name : '') }}">
                @if($errors->has('name'))
            ...
            <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                ...
                <input type="email" required id="email" name="email" class="form-control" value="{{ old('email', isset($user) ? $user->email : '') }}">
                @if($errors->has('email'))
                    <em class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                ...
                <select  name="departments[]" id="departments" class="form-control select2" multiple="multiple">
                    @foreach($departments as $id => $department)
                        <option value="{{ $id }}" {{ (in_array($id, old('departments', [])) || isset($user) && $user->departments->contains($id)) ? 'selected' : '' }}>
                            {{ $department}}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                ...
                <select  name="offices[]" id="offices" class="form-control select2" multiple="multiple">
                    @foreach($offices as $id => $office)
                        <option value="{{ $id }}" {{ (in_array($id, old('offices', [])) || isset($user) && $user->offices->contains($id)) ? 'selected' : '' }}>
                            {{ $office}}
                        </option>
                    @endforeach
                </select>
